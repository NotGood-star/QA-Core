...
/* =========================================================
   ...
   Keeps Discord custom_id values short.
========================================================= */
...
function hasDeveloperRole(member) {
  return hasRole(member, CONFIG.roles.developer);
}

function hasTesterRole(member) {
  return hasRole(member, CONFIG.roles.tester);
}
    ...
    .addComponents(
      ...
      new ActionRowBuilder().addComponents(
        new TextInputBuilder()
          ...
          .setPlaceholder("Example: Start: 8 PM | End: 9 PM")
          .setStyle(TextInputStyle.Short)
          .setRequired(true)
          .setMaxLength(200)
      )
    ...
    new StringSelectMenuBuilder()
      ...
      .addOptions(
        new StringSelectMenuOptionBuilder()
          .setLabel("Paid Test")
          .setDescription("Testers receive a Robux reward.")
          .setValue("paid")
          .setEmoji(CONFIG.emojis.qa_paid),

        new StringSelectMenuOptionBuilder()
          .setLabel("Volunteer Test")
          .setDescription("Free testing with no Robux reward.")
          .setValue("volunteer")
          .setEmoji(CONFIG.emojis.qa_free)
      )
  );
}
    ...
    .addComponents(
      new ActionRowBuilder().addComponents(
        new TextInputBuilder()
          ...
          .setPlaceholder("Example: 50 Robux per tester")
          .setStyle(TextInputStyle.Short)
          .setRequired(true)
          .setMaxLength(100)
      )
...
function buildTesterListEmbed(test, testers) {
  ...
  return {
    color: 0x6c4dff,
    title: `${e.qa_tester} Testers — #${test.id}`,
    description:
...
client.on("interactionCreate", async (interaction) => {
  try {
    ...
    if (interaction.isChatInputCommand()) {
      ...
      if (subcommand === "setup") {
        if (!canManageQA(interaction.member)) {
      ...
      if (subcommand === "close") {
        ...
        const isHost =
          test.host_id === interaction.user.id;

        if (
          !isHost &&
          !canManageQA(interaction.member)
        ) {
          return interaction.reply({
            content:
    ...
    if (
      ...
      interaction.customId === "qa_host_start"
    ) {
      if (
        !hasDeveloperRole(interaction.member) &&
        !canManageQA(interaction.member)
      ) {
        return interaction.reply({
          content:
      ...
      return interaction.showModal(buildHostModal());
    }
    ...
    if (
      ...
      interaction.customId === "qa_host_modal"
    ) {
      if (
        !hasDeveloperRole(interaction.member) &&
        !canManageQA(interaction.member)
      ...
      const gameLink = cleanText(
        interaction.fields.getTextInputValue("game_link"),
        300
      );

      const maxTestersRaw = cleanText(
        interaction.fields.getTextInputValue("max_testers"),
        10
      );
      ...
      const schedule = cleanText(
        interaction.fields.getTextInputValue("schedule"),
        200
      );

      const maxTesters = Number(maxTestersRaw);
      ...
      return interaction.reply({
        ...
        components: [buildTypeSelect(draftId)],
        ephemeral: true
      });
    }
    ...
    if (
      ...
      interaction.customId.startsWith("qa_type:")
    ) {
      const draftId =
      ...
      if (type === "paid") {
        ...
        return interaction.showModal(
          buildRewardModal(draftId)
        );
      }

      draft.testType = "volunteer";
      draft.reward = "Volunteer / No Robux Reward";
    ...
    if (
      interaction.isModalSubmit() &&
      interaction.customId.startsWith("qa_reward:")
    ) {
      const draftId =
        interaction.customId.split(":")[1];
    ...
    if (
      interaction.isButton() &&
      interaction.customId.startsWith("qa_join:")
    ) {
      const id = Number(
        interaction.customId.split(":")[1]
      );
      ...
      if (
        !hasTesterRole(interaction.member) &&
        !hasDeveloperRole(interaction.member) &&
        !canManageQA(interaction.member)
      ) {
        return interaction.reply({
          content:
            `${CONFIG.emojis.qa_cross} You need the **Tester** role to join a test.`,
          ephemeral: true
        });
      }

      const result = joinTest(
        id,
        interaction.user.id
      );
    ...
    if (
      interaction.isButton() &&
      interaction.customId.startsWith("qa_testers:")
    ) {
      const id = Number(
        interaction.customId.split(":")[1]
      );
      ...
      const testers = listTesters(id);

      return interaction.reply({
        embeds: [
          buildTesterListEmbed(
            test,
            testers
          )
        ],
    ...
    if (
      interaction.isButton() &&
      interaction.customId.startsWith("qa_close:")
    ) {
      const id = Number(
        interaction.customId.split(":")[1]
      );
      ...
      if (
        ...
        return interaction.reply({
          content:
            `${CONFIG.emojis.qa_cross} Only the host or QA staff can close this test.`,
          ephemeral: true
        });
      }
